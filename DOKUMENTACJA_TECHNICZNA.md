...
- Nowa, zintegrowana ikona główna `Icon_App_Health_IT.png` przyporządkowywana jest dla całego spektrum formatów w PWA oraz na ekranie głównym (Splash Screenie) podczas startu aplikacji. Dodatkowo zoptymalizowano proces budowy manifestu dostosowując limit wagi plików do cache (`maximumFileSizeToCacheInBytes`), aby obsłużyć pliki wysokiej rozdzielczości bez kompresji.
